    ...
    <>
      <div>
        <button onClick={toggleCart}>
          <ShoppingCart className=" text-gray-600 group-hover:text-secondary transition-colors duration-200" />
        </button>
      </div>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 h-screen bg-black opacity-60 z-40 transition-opacity duration-300"
          onClick={toggleCart}
        />
      )}
      ...
      <div
        ...
        className={`fixed top-0 right-0 h-dvh w-full max-w-md bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-out flex-col ${
          isOpen ? "flex" : "hidden"
        }`}
      >
        {/* Header */}
