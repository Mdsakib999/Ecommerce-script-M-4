    ...
    <Link
      ...
      className={`flex mx-auto items-center ease-in-out duration-300 transition-all  text-md justify-center w-max rounded-md  ${
        type === "secondary"
          ...
          : type === "outline"
          ? " bg-transparent  text-purple-400 mr-4 font-semibold border-2 border-transparent hover:border-b-purple-400 pb-1 pt-2 rounded-none transform transition-colors duration-300"
          : type === "danger"
          ? "bg-transparent text-red-500 font-semibold border-3 border-[#FF595E] rounded-lg"
          : ""
      }
      ${isActive ? "text-purple-500 border-b-purple-500" : ""} `}
    >
      {children}
    </Link>
  );
